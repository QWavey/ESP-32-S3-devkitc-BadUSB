...
void executeCommand(String line) {
  ...
  // Handle function-style variable execution (variable name followed by parentheses)
  if (line.endsWith("()") && variables.find(line.substring(0, line.length() - 2)) != variables.end()) {
    String functionName = line.substring(0, line.length() - 2);
    String functionCommand = variables[functionName];
    Serial.println("Executing function: " + functionName + " -> " + functionCommand);
    executeCommand(functionCommand);
    return;
  }
  ...
  if (line.indexOf('=') > 0 && !line.startsWith("STRING") && !line.startsWith("REM") && 
      ...
      !line.startsWith("WIFI_SCAN_TIME") && !line.startsWith("VAR ")) {
...
void setupWebServer() {
  ...
  server.on("/", []() {
    ...
    html += "<pre>username = admin\npassword = 1234\nSTRING username\nTAB\nSTRING password\nENTER\n\nVAR greeting = Hello\nSTRING greeting\nREM Output: Hello</pre>";
    html += "<button onclick='loadExample(2)'>Load Example</button>";
    
    html += "<h4>🔧 Function Variables:</h4>";
    html += "<pre>OpenNotepad = GUI r\nTypeHello = STRING Hello World!\nWaitTime = DELAY 2000\n\nOpenNotepad()\nWaitTime()\nSTRING notepad\nENTER\nWaitTime()\nTypeHello()</pre>";
    ...
    html += "    2: 'username = admin\\npassword = 1234\\nSTRING username\\nTAB\\nSTRING password\\nENTER\\n\\nVAR greeting = Hello\\nSTRING greeting\\nREM Output: Hello',";
    html += "    3: 'OpenNotepad = GUI r\\nTypeHello = STRING Hello World!\\nWaitTime = DELAY 2000\\n\\nOpenNotepad()\\nWaitTime()\\nSTRING notepad\\nENTER\\nWaitTime()\\nTypeHello()',";
